...
for col in cols + ['park_factor_SLG','park_factor_OBP']:
...
df.reset_index(inplace=True, drop=True)   
df.reset_index(inplace=True, drop=True)
#df['next_game'] = np.where(df['id']==df['id'].shift(-1), df['game'].shift(-1), np.nan)
...
df[df['pts'].notnull()].to_pickle('./batters.pkl')
